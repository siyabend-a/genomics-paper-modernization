...
## Content
This independent pipeline project is dedicated to align the reads that **Ouyang et al. (2009)** used to the modern mm39 mouse genome and apply **Ouyang et al. (2009)**'s predictive model to the new data to test reproducibility. **Ouyang et al. (2009)**'s study predicts gene expression(RNA-seq data) from ChIP-Seq data of 12 transcription factors, and uses an exponential decay for transcription association calculation instead of a binary one. This repository contains the scripts used for the project and the scripts for downloading the data.

## Current status
![Current Fit from alignment to mm39](regression.png)
**Figure 1:** The figure shows the predicted gene expression(RNA-Seq) on the y-axis and the actual gene expression on the x-axis. R<sup>2</sup> = 0.266
